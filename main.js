...
{
	var win = window.open( 'https://store.steamchina.com/gotflash','gotflash','width=536,height=546,resize=yes,scrollbars=yes');
	win.focus();
}
...
function GameHover( elem, event, divHover, rgHoverData )
{
	...
	else if ( ( !$Hover.is(':visible') || g_HoverState.target != $Elem[0] ) && !oElemState.timer )
	{
		...
		if ( !$HoverData.length && !oElemState.bAjaxRequestMade )
		{
			...
			{
				// is cc needed?
				rgAjaxParams = { /*cc: rgHoverData['cc'],*/ l: 'sc_schinese' };
			}
			if ( rgHoverData['v6'] )
			{
...
function MobileApp_ShowSearchResults( $SNR, $Term )
{
	...
	window.location = 'https://store.steamchina.com/search/?term=' + $Term + snr;
}
...
function AddFreeLicense( subid, strDisplayName )
{
	...
	{
		posts.push( $J.post( 'https://store.steamchina.com/checkout/addfreelicense/' + subids[i], { ajax: true, sessionid: g_sessionID }) );
	}
...
function AddFreeBundle( bundleid, strDisplayName )
{
	...
	var posts = [];
	posts.push( $J.post( 'https://store.steamchina.com/checkout/addfreebundle/' + bundleid, { ajax: true, sessionid: g_sessionID }) );
