...
GDynamicStore = {
	...
	CapsuleSettingsMenu: function( elSource )
	{
		...
		if( strAppIDs )
		{
			...
			var $elPreferences = $J ( '<a/>' ).attr('href', 'https://store.steamchina.com/account/preferences' ).text ( "\u504f\u597d" ).addClass( 'option' );
	...
	ModifyWishlist: function( $elSource, appid, bRemove, fnOnSuccess, fnOnFail )
	{
		...
		if( bRemove )
			url = 'https://store.steamchina.com/api/removefromwishlist';
	...
	PopulateRecommendedTagList: function()
	{
		...
		for( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 4; i++ )
		{
			...
			var url = 'https://store.steamchina.com/tags/zh-cn/' + encodeURIComponent( tag.name );
...
GStoreItemData = {
	...
	GetAppURL: function( unAppID, strFeatureContext, nDepth, nCuratorClanID)
	{
		if ( typeof GStoreItemData.rgAppData[ unAppID ] == 'object' && 'url_name' in GStoreItemData.rgAppData[ unAppID ] ) {
			return GStoreItemData.AddNavEventParamsToURL( 'https://store.steamchina.com/app/' + unAppID + '/' + GStoreItemData.rgAppData[ unAppID ].url_name + '/', strFeatureContext, nDepth, nCuratorClanID )
		}

		return GStoreItemData.AddNavEventParamsToURL( 'https://store.steamchina.com/app/' + unAppID + '/', strFeatureContext, nDepth, nCuratorClanID )
	},

	GetPackageURL: function( unPackageID, strFeatureContext, nDepth )
	{
		return GStoreItemData.AddNavEventParamsToURL( 'https://store.steamchina.com/sub/' + unPackageID + '/', strFeatureContext, nDepth )
	},

	GetBundleURL: function( unBundleID, strFeatureContext, nDepth )
	{
		return GStoreItemData.AddNavEventParamsToURL( 'https://store.steamchina.com/bundle/' + unBundleID + '/', strFeatureContext, nDepth )
	},
...
{
	$J.get(
		'https://store.steamchina.com/explore/howitworks/',
		{
			l : 'sc_schinese'
		},
		function( data )
...
{
	return "https:\/\/avatars.cdn.steamchina.eccdnx.com\/" + strHash + strSize + '.jpg';
}

function GetScreenshotURL( appid, filename, sizeStr )
{
	if( sizeStr )
		return 'https://cdn.steamchina.eccdnx.com/steam/' + 'apps/' + appid + '/' + filename.replace('.jpg', sizeStr + '.jpg');

	return 'https://cdn.steamchina.eccdnx.com/steam/' + 'apps/' + appid + '/' + filename;
}
...
function UpdateStoreBannerForAdditionalCartDiscount( nCartDiscount )
{
	...
	<div class="placeHolder_lunarSale2019_giftActiveBar">	\
		...
		<div class="lunar_sale_title"><img src="https://store.steamchina.eccdnx.com/public/images/promo/lunar2019/lny2019_title_zh-cn.png"/></div> \
